...
class VotersController extends Controller
{
    ...
    public function index()
    {
    	...
    	$unVote = DB::table('pemilih')->where('status', 2)
    				->join('tbl_status','pemilih.status', '=' , 'tbl_status.id')
    				->select('pemilih.*','tbl_status.nama')
    				->get();
    	$inVote = DB::table('pemilih')->where('status', 1)
    				->join('tbl_status','pemilih.status', '=' , 'tbl_status.id')
    				->select('pemilih.*','tbl_status.nama')
    				->get();

    	return view('dashboard/voter/voters',['pemilih' => $pemilih, 'unVote' => $unVote, 'inVote' => $inVote]);
    }
